...
*/
__global__ void findMax_kernel(Data_Type *g_idata, Data_Type *g_oValue, unsigned int *g_oIndex, unsigned int size, int abs) {
    __shared__ volatile Data_Type s_value[BLOCK_SIZE];
...
*/
__global__ void calculate_scale_moment_principal_solution_kernel(Data_Type *ihsmmpsf, Data_Type *smresidual, Data_Type *smpsol, unsigned int nmoments, unsigned int size)
{
    unsigned int tid = threadIdx.x;
    unsigned int idx = blockIdx.x*BLOCK_SIZE + tid;

    extern __shared__ volatile Data_Type s_ihsmmpsf[];
    Data_Type l_smresidual[LOCAL_M_SIZE];
...
__global__ void get_mval_kernel(Data_Type *smpsol, Data_Type *smresidual, Data_Type* mval,unsigned int *shape)
{
...
*/
__global__ void update_moment_model_residual_kernel(Data_Type *m_model, Data_Type* scalestack, Data_Type* smresidual, Data_Type *ssmmpsf,
Data_Type* mval, unsigned int* para, Data_Type gain, unsigned int nmoments, unsigned int nscales,unsigned int mscale, unsigned int px, unsigned int py, unsigned int nx, unsigned int ny )
{
...
__global__ void pmax_residual_kernel(Data_Type* smresidual, Data_Type *re_smres, Data_Type pmax, unsigned int size)
{
    unsigned int tid = threadIdx.x;
    unsigned int idx = blockIdx.x*BLOCK_SIZE + tid;
...
def cuda_findMax_2D(gpu_array,arraySize, absmax, getIndex=False):
    ...
    cpu_value = numpy.zeros(BLOCK_Count,dtype=Data_Type,order='C')
...
def cuda_findMax(A, absmax=False, getIndex=False):
    ...
    for i in range (nm):
        gpu_A = gpuarray.to_gpu(A[i,:,:].astype(Data_Type))
...
def cuda_find_global_optimum_default(ihsmmpsf, smresidual, windowstack):
    ...
    sy = 0
    maxv = 0.0
    gpu_mavl = gpuarray.zeros((n),dtype=Data_Type)
    gpu_smpsol = gpuarray.zeros((n,x,y),dtype=Data_Type)   
    for i in range (s):
...
def cuda_update_moment_model_residual(gpu_m_model, gpu_scalestack, gpu_smresidual, gpu_ssmmpsf, para, gain, mscale, gpu_mval):
    ...
    Data_Type(gain), numpy.uint32(nmoments), numpy.uint32(nscales), numpy.uint32(mscale), numpy.uint32(px), numpy.uint32(py), numpy.uint32(nx),numpy.uint32(ny),\
...
def cuda_return_res_zero(gpu_smresidual, pmax):
    ...
    gpu_res = gpuarray.zeros((nmoments, nx, ny),dtype=Data_Type)
    cuda_pmax_residual_core(gpu_smresidual[0,:,:,:], gpu_res, Data_Type(pmax), numpy.uint32(asize), block=(thread1,1,1),grid=(grid1,1))
...
def cuda_msmfsclean(dirty, psf, window, gain, thresh, niter, scales, fracthresh, findpeak='CASA'):
    ...
    gpu_m_model  = gpuarray.zeros(dirty.shape,dtype=Data_Type)
    ...
    start1 = time.time()  
    gpu_pscalestack= gpuarray.to_gpu_async(pscalestack.astype(Data_Type))
    ...
    start1 = time.time() 
    gpu_smresidual = gpuarray.to_gpu_async(smresidual.astype(Data_Type))
    ...
    start1 = time.time() 
    gpu_ssmmpsf = gpuarray.to_gpu_async(ssmmpsf.astype(Data_Type))
    gpu_ihsmmpsf = gpuarray.to_gpu_async(ihsmmpsf.astype(Data_Type))
    time3 = time.time()  - start1
    print ("Time used for msmfsclean copy data: %f s " % (time1+time2+time3))
    ...
    for i in range(niter):
        ...
        if peak < absolutethresh:
            log.info("msmfsclean: Absolute value of peak %.6f is below stopping threshold %.6f at loop %f" \
                     % (peak, absolutethresh,i))
            break
    ...
    return m_model.astype(numpy.float64), res_smre.astype(numpy.float64)
