...
from arl.pyopencl_util.reikna_fft import reikna_fftn
from arl.pyopencl_util.pyocl_gridding import ocl_convolutional_grid
...
def predict_2d_base(vis: Visibility, model: Image, **kwargs) -> Visibility:
    ...
    print("Time used for predict  coalesce_visibility: %f s" % elapsed)
    ...
    print("Time used for predict get_map_kernel_list: %f s" % (time.time() - start))
    ...
    flop = vuvwmap.shape[0]*20 + avis.data['vis'].size * 32*32*10
    dataacc = vuvwmap.shape[0]*7*8 + (model.data.size + 8*8*32*32)*16
    gflops = (flop/elapsed)/1e9
    bandw = (dataacc/elapsed)/(1024*1024*1024)
    fft_gflops = 4*model.data.size *(math.log(model.data.size,2) + 2) / (start2 - start1)/1e9
    fft_bdwith =  ((model.data.size*8 + uvgrid.size*16)/(start2 - start1))/(1024*1024*1024)
    print("Time used for fft: %f s, gflops = %f ,bandwidth = %f GB/s" % ((start2 - start1),fft_gflops,fft_bdwith))
    print("Time used for convolutional_degrid: %f s, gflops =%f , bandwithd=%f GB/s" % (elapsed,gflops,bandw))
    ...
    if type(vis) is not Visibility:
        start = time.time()
        bvis =  decoalesce_visibility(svis)
        elapsed = (time.time() - start)
        print("Time used for predict  decoalesce visibility: %f s" % elapsed)
        return bvis
    else:
        return svis
...
def predict_2d(vis: Visibility, im: Image, **kwargs) -> Visibility:
    ...
    start = time.time()
    p2 = predict_2d_base(vis, im, **kwargs)
    elapsed = (time.time() - start)
    print("Time used for predict_2d_base : %f s" % elapsed)
    return p2
...
def invert_2d_base(vis: Visibility, im: Image, dopsf: bool = False, normalize: bool = True, **kwargs) \
    ...
    elapsed = (time.time() - start)
    print("Time used for invert coalesce_visibility: %f s" % elapsed)
    ...
    print("Time used for invert get_map_kernel_list: %f s" % (time.time() - start))
    ...
    start = time.time()
    imgridpad, sumwt = ocl_convolutional_grid(vkernellist, imgridpad, svis.data['vis'],
                                          svis.data['imaging_weight'],
                                          vuvwmap,
                                          vfrequencymap, vpolarisationmap)

    elapsed = (time.time() - start)        
    flop = vuvwmap.shape[0]*20 + svis.data['vis'].size * (32*32*10 + 3)
    dataacc = vuvwmap.shape[0]*(5 + npol*2 )*8 + (nchan*npol*nx*ny + 8*8*32*32)*16 + nchan*npol*8
    gflops = (flop/elapsed)/1e9
    bandw = (dataacc/elapsed)/(1024*1024*1024)
    print("Time used for convolutional_grid: %f s, gflops =%f , bandwithd=%f GB/s" % (elapsed,gflops,bandw))
    ...
    if imaginary:
        ...
        result = extract_mid(reikna_fftn(imgridpad,1) * gcf, npixel=nx)
        ...
        elapsed = (time.time() - start)
        gflops = 4*imgridpad.size *(math.log(imgridpad.size,2) + 1 )/ elapsed/1e9
        ifft_datawd = (imgridpad.size*16 + result.size*8)/elapsed/(1024*1024*1024)
        print("Time used for ifft: %f s, gflops = %f, bandwidth = %f GB/s" % (elapsed,gflops,ifft_datawd))
...
def invert_2d(vis: Visibility, im: Image, dopsf=False, normalize=True, **kwargs)  -> (Image, numpy.ndarray):
    ...
    start = time.time()
    i2 = invert_2d_base(vis, im, dopsf, normalize=normalize, **kwargs)
    elapsed = (time.time() - start)
    print("Time used for invert_2d_base: %f s" % (elapsed))
    return i2
